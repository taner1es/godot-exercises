...
func _ready():
	...
	var gun_aim_points_pos = $Rotation_Helper/Gun_Aim_Point.global_transform.origin
	
	for weapon in weapons:
		var weapon_node = weapons[weapon]
		if weapon_node != null:
			weapon_node.player_node = self
			weapon_node.look_at(gun_aim_points_pos, Vector3(0, 1, 0))
...
func process_input(delta):
	...
	if Input.get_connected_joypads().size() > 0:
		...
		if OS.get_name() == "Windows":
			...
			#line written for tutorial disabled because didn't work fine
			#joypad_vec = Vector2(Input.get_joy_axis(0, 1), Input.get_joy_axis(0, 2))
			#I changed a very little
			joypad_vec = Vector2(Input.get_joy_axis(0, JOY_ANALOG_LX), -Input.get_joy_axis(0, JOY_ANALOG_LY))
...
func process_view_input(delta):
	...
	if Input.get_connected_joypads().size() > 0:
		...
		if OS.get_name() == "Windows":
			...
			# I changed a very little
			joypad_vec = Vector2(Input.get_joy_axis(0, JOY_ANALOG_RX), Input.get_joy_axis(0, JOY_ANALOG_RY))
		elif OS.get_name() == "OSX":
			joypad_vec = Vector2(Input.get_joy_axis(0, 3), Input.get_joy_axis(0, 4))
